...
Eine **Home-Assistant-Custom-Integration** (über HACS installierbar) für den dynamischen
österreichischen Stromtarif **smartTIMES** von smartENERGY. Sie ruft die öffentliche
Tarifpreis-API ab (kein API-Key) und stellt stündliche/viertelstündliche Preise als Sensoren
bereit – inklusive aller variablen Nebenkosten (Steuern, Abgaben, Netzentgelte) und eines
Binary-Sensors, der die günstigsten Stunden des Tages markiert.
...
python3.13 -m py_compile custom_components/smarttimes/*.py
```
...
- **Brutto/Netto & USt.**: Preise werden so gespeichert, wie die API sie liefert – **brutto
  ...
  **Gesamtpreis** = Arbeitspreis + Nebenkosten, wobei **netto summiert** und die USt. **einmal
  ...
  `surcharges.py`/`grid_fees.py` sind daher **netto** hinterlegt.
...
- **`config_flow.py`** – UI-Einrichtung (kein YAML): Haupteintrag (USt., Netzgebiet) + Options-Flow
  + Subentry-Flow für die Günstige-Stunde-Sensoren. `single_config_entry: true` → nur **eine**
  Instanz. Schemas müssen frontend-serialisierbar bleiben (keine Lambdas/`vol.All` mit Callable;
  Validierung stattdessen im Flow-Schritt).

## Wichtige Hinweise

- **Zeitzone**: Die API liefert **lokale** Zeitstempel ohne Offset; HA sollte auf `Europe/Vienna`
  stehen. `api.py` fällt beim Parsen auf `dt_util.DEFAULT_TIME_ZONE` zurück.
- **Domain** ist `smarttimes` (siehe `const.py`/`manifest.json`).
...
1. Version in **`custom_components/smarttimes/manifest.json`** anheben (SemVer; Minor = neue
   abwärtskompatible Features).
2. Mergen nach `main`.
